= Polkabot - Polkadot Network Crawler
...
----
{
  "version":"https://schema.emeraldpay.io/polkabot", <1>
  "timestamp":"2020-03-06T05:07:42.280046Z", <2>
...
----
<1> Version id for this file structure. If the schema of the file changed that breaks compatibility (i.e., fields are
moved or renamed) - it gets a new unique id. Please note that at this stage of the development, the schema is not yet
finalized.
<2> The timestamp when log item was written. Please note it can be different from connection time.
<3> `IN` or `OUT`, depending on the source of the connection. `IN` means that the remote node initiated connection to the bot.
<4> List of protocols (in terms of libp2p) supported by the remote peer.
